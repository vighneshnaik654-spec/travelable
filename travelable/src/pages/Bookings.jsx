...
export default function Bookings() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 space-y-3">
      <h1 className="text-3xl font-bold text-green">My bookings</h1>
      ...
      {list?.map((b) => (
        <article key={b.id} className="border rounded-xl p-4">
          ...
          {b.status !== "Cancelled" && <button onClick={() => cancel(b.id)} className="mt-2 rounded-lg text-red-500 border px-4 py-2">Cancel booking</button>}
        </article>
      ))}
    </div>
  );
